...
export function BillEditor() {
  ...
  const hasEnoughData = bill.people.length > 0 && bill.items.length > 0;
  ...
  return (
    <div className="w-full max-w-2xl space-y-5">
      <div className="flex items-center justify-between gap-3 rounded-2xl border border-zinc-200/80 bg-white/70 px-5 py-4 shadow-sm backdrop-blur-sm dark:border-zinc-800 dark:bg-zinc-900/60">
        <input
          value={bill.title}
          onChange={(e) => dispatch({ type: "SET_TITLE", title: e.target.value })}
          aria-label="Bill title"
          className="min-w-0 flex-1 truncate rounded-md bg-transparent text-xl font-semibold text-zinc-900 outline-none placeholder:text-zinc-400 focus:ring-2 focus:ring-indigo-500/40 dark:text-zinc-50"
        />
      ...
      <section className="rounded-2xl border border-zinc-200/80 bg-white/70 p-5 shadow-sm backdrop-blur-sm dark:border-zinc-800 dark:bg-zinc-900/60">
        <Results people={bill.people} result={effectiveResult} loading={loading} />
      </section>
    </div>
  );
}
